...
class ImportServiceTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.source_tree = Tree.objects.create(
            name='Исходное дерево',
            description='Описание исходного дерева'
        )
    ...
    def test_import_handles_duplicates(self) -> None:
        ...
        persons_with_same_name = target_tree.persons.filter(
            first_name='Пётр',
            last_name='Иванов'
        )
    ...
    def test_import_creates_owner_collaborator(self) -> None:
        ...
        tree = task.tree
        owner_collaborator = tree.collaborators.filter(
            user=self.user,
            role=CollaboratorRoleEnum.OWNER
        ).first()
