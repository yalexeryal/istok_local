...
class TreeAdminTest(TestCase):
    ...
    def test_tree_change_view(self) -> None:
        ...
        response = self.client.get(reverse("admin:genealogy_tree_change", args=[self.tree.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Тестовое дерево")
...
class PersonAdminTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.person = Person.objects.create(
            first_name="Иван", last_name="Иванов", gender=GenderEnum.MALE, birth_date=date(1990, 1, 1), tree=self.tree
        )
    ...
    def test_person_change_view(self) -> None:
        ...
        response = self.client.get(reverse("admin:genealogy_person_change", args=[self.person.pk]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_person_search(self) -> None:
        """Тест поиска персон."""
        response = self.client.get(reverse("admin:genealogy_person_changelist") + "?q=Иванов")
        self.assertEqual(response.status_code, 200)
...
class UserProfileAdminTest(TestCase):
    ...
    def test_profile_change_view(self) -> None:
        ...
        response = self.client.get(reverse("admin:genealogy_userprofile_change", args=[self.profile.pk]))
        self.assertEqual(response.status_code, 200)
...
class UserAdminIntegrationTest(TestCase):
    ...
    def test_signal_creates_profile_on_user_creation(self) -> None:
        ...
        new_user = User.objects.create_user(username="signaltest", password="pass123")
    ...
    def test_signal_creates_profile_on_superuser_creation(self) -> None:
        ...
        new_superuser = User.objects.create_superuser(
            username="newsuperuser", email="super@example.com", password="superpass123"
        )
    ...
    def test_profile_default_tier_is_free(self) -> None:
        ...
        """
        new_user = User.objects.create_user(username="freetieruser", password="pass123")
        self.assertEqual(new_user.profile.tier, UserTierEnum.FREE)
    ...
    def test_profile_not_created_twice(self) -> None:
        ...
        """
        new_user = User.objects.create_user(username="nosignalduplicate", password="pass123")
