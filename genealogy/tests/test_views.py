...
class TreeListViewTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.tree = Tree.objects.create(name="Семья Ивановых", description="Родовое дерево семьи Ивановых")
        TreeCollaborator.objects.create(tree=self.tree, user=self.user, role=CollaboratorRoleEnum.OWNER)

        for i in range(5):
            Person.objects.create(first_name=f"Person{i}", last_name="Иванов", tree=self.tree)
    ...
    def test_collaborator_sees_tree(self) -> None:
        ...
        TreeCollaborator.objects.create(tree=self.tree, user=editor, role=CollaboratorRoleEnum.EDITOR)
...
class PersonCRUDViewsTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.person = Person.objects.create(first_name="Иван", last_name="Иванов", gender="male", tree=self.tree)
    ...
    def test_create_person_success(self) -> None:
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(Person.objects.filter(first_name="Мария", last_name="Петрова", tree=self.tree).exists())
...
class RelationshipCRUDViewsTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.relationship = Relationship.objects.create(
            from_person=self.father, to_person=self.son, relationship_type="biological_parent"
        )
    ...
    def test_create_relationship_success(self) -> None:
        ...
        self.assertTrue(
            Relationship.objects.filter(
                from_person=mother, to_person=self.son, relationship_type="biological_parent"
            ).exists()
        )
...
class LifeEventCRUDViewsTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.event = LifeEvent.objects.create(person=self.person, event_type="birth", event_date="1990-01-01")
    ...
    def test_create_event_success(self) -> None:
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(LifeEvent.objects.filter(person=self.person, event_type="education", location="МГУ").exists())
    ...
    def test_delete_success(self) -> None:
        ...
        response = self.client.post(reverse("genealogy:life_event_delete", args=[event_pk]))
        self.assertEqual(response.status_code, 302)
