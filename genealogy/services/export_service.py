...
class ExportService:
    ...
    def _export_full(self, tree: Tree) -> tuple[bytes, int]:
        ...
        with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            zip_file.writestr(
                'persons.json',
                json.dumps(persons_data, indent=2, ensure_ascii=False)
            )

            # Relationships
            relationships = Relationship.objects.filter(
                from_person__tree=tree
            )
            relationships_data = [
            ...
            zip_file.writestr(
                'relationships.json',
                json.dumps(relationships_data, indent=2, ensure_ascii=False)
            )

            # Life Events
            life_events = LifeEvent.objects.filter(person__tree=tree)
            life_events_data = [
                self._life_event_to_dict(e) for e in life_events
            ]
            zip_file.writestr(
                'life_events.json',
                json.dumps(life_events_data, indent=2, ensure_ascii=False)
            )

            # Collaborators
            collaborators = tree.collaborators.all()
            collaborators_data = [
                self._collaborator_to_dict(c) for c in collaborators
            ]
            zip_file.writestr(
                'collaborators.json',
                json.dumps(collaborators_data, indent=2, ensure_ascii=False)
            )
                    ...
                    try:
                        ...
                        if photo_path.exists():
                            with open(photo_path, 'rb') as f:
                                zip_file.writestr(
                                    f'media/photos/{person.photo.name}',
                                    f.read()
                                )
    ...
    def _export_relative(
        ...
        with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            for person in relatives:
                ...
                person_data = self._person_to_dict_with_privacy(
                    person,
                    privacy_settings,
                    degree,
                    privacy_overrides
            ...
            relationships = Relationship.objects.filter(
                from_person_id__in=relative_ids,
                to_person_id__in=relative_ids
            )
            relationships_data = [
                self._relationship_to_dict(r) for r in relationships
            ]
            ...
            for person in relatives:
                if person.photo and not self._should_hide_photos(
                        person, privacy_settings, degrees.get(person.pk, float('inf')),
                        privacy_overrides
                ):
                    try:
                        photo_path = Path(person.photo.path)
    ...
    def _should_hide_photos(
        ...
        if privacy_overrides and privacy_overrides.get('hide_photos'):
            return True
