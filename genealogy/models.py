...
class Person(models.Model):
    ...
    status = models.CharField(max_length=20, choices=PersonStatusEnum.choices, default=PersonStatusEnum.SANDBOX,
                              verbose_name='Статус')
    merged_into = models.ForeignKey('self', on_delete=models.SET_NULL, blank=True, null=True,
                                    related_name='merged_persons', verbose_name='Объединена с')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='Дата создания')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='Дата обновления')
    updated_by = models.ForeignKey(User, on_delete=models.SET_NULL, blank=True, null=True,
                                   related_name='updated_persons', verbose_name='Кем обновлено')
    sync_version = models.IntegerField(default=0, verbose_name='Версия синхронизации')
    last_synced_at = models.DateTimeField(null=True, blank=True, verbose_name='Последняя синхронизация')
...
class LifeEvent(models.Model):
    ...
    description = models.TextField(blank=True, null=True, verbose_name='Описание')
    related_person = models.ForeignKey(Person, on_delete=models.SET_NULL, blank=True, null=True,
                                       related_name='related_events', verbose_name='Связанная персона')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='Дата создания')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='Дата обновления')
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, blank=True, null=True,
                                   related_name='created_events', verbose_name='Кем создано')
    sync_version = models.IntegerField(default=0, verbose_name='Версия синхронизации')
    last_synced_at = models.DateTimeField(null=True, blank=True, verbose_name='Последняя синхронизация')
...
class Relationship(models.Model):
    ...
    updated_at = models.DateTimeField(auto_now=True, verbose_name='Дата обновления')
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, blank=True, null=True,
                                   related_name='created_relationships', verbose_name='Кем создано')
    sync_version = models.IntegerField(default=0, verbose_name='Версия синхронизации')
    last_synced_at = models.DateTimeField(null=True, blank=True, verbose_name='Последняя синхронизация')
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=['from_person', 'to_person', 'relationship_type'],
...
class TreeCollaborator(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='tree_collaborations',
                             verbose_name='Пользователь')
    role = models.CharField(max_length=20, choices=CollaboratorRoleEnum.choices, default=CollaboratorRoleEnum.VIEWER,
                            verbose_name='Роль')
...
class ChangeRequest(models.Model):
    ...
    owner = models.ForeignKey(User, on_delete=models.CASCADE, related_name='owned_change_requests',
    ...
    status = models.CharField(max_length=20, choices=ChangeRequestStatusEnum.choices,
                              default=ChangeRequestStatusEnum.PENDING, verbose_name='Статус')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='Дата создания')
...
class ExportTask(models.Model):
    ...
    export_format = models.CharField(max_length=20, choices=ExportFormatEnum.choices, default=ExportFormatEnum.JSON_ZIP,
                                     verbose_name='Формат экспорта')
    target_person = models.ForeignKey(Person, on_delete=models.SET_NULL, null=True, blank=True,
                                      related_name='export_tasks', verbose_name='Целевая персона')
    privacy_overrides = models.JSONField(default=dict, blank=True, verbose_name='Переопределение настроек приватности')
    status = models.CharField(max_length=20, choices=ExportStatusEnum.choices, default=ExportStatusEnum.PENDING,
                              verbose_name='Статус')
...
class ImportTask(models.Model):
    ...
    tree = models.ForeignKey(Tree, on_delete=models.SET_NULL, null=True, blank=True, related_name='import_tasks',
                             verbose_name='Дерево')
    ...
    status = models.CharField(max_length=20, choices=ImportStatusEnum.choices, default=ImportStatusEnum.PENDING,
                              verbose_name='Статус')
...
class UserProfile(models.Model):
    ...
    tier = models.CharField(max_length=20, choices=UserTierEnum.choices, default=UserTierEnum.FREE,
                            verbose_name='Тариф')
